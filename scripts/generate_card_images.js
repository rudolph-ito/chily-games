...
for (const element of elements) {
  if (
    element.match(/^club_/) ||
    element.match(/^diamond_/) ||
    element.match(/^heart_/) ||
    element.match(/^spade_/) ||
    element === "joker_red.png"
  ) {
    output[element.replace(".png", "")] = readFileAsBase64(
      path.join(root, element)
    );
  }
}
...
fs.writeFileSync(
  "frontend/src/app/data/card_images.js",
