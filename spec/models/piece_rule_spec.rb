...
describe PieceRule do
  ...
  context '#movement' do
    ...
    context 'no movement_maximum' do
      let(:piece_rule) { build(:piece_rule, movement_minimum: 1, movement_maximum: nil ) }
      specify { piece_rule.movement.should == "Orthogonal line - 1 or more space(s)" }
    end

    context 'movement_minimum == movement_maximum' do
      let(:piece_rule) { build(:piece_rule, movement_minimum: 2, movement_maximum: 2 ) }
      specify { piece_rule.movement.should == "Orthogonal line - 2 space(s)" }
    end

    context 'movement_minimum < movement_maximum' do
      let(:piece_rule) { build(:piece_rule, movement_minimum: 1, movement_maximum: 2 ) }
      specify { piece_rule.movement.should == "Orthogonal line - 1 to 2 space(s)" }
    end
  end
end
