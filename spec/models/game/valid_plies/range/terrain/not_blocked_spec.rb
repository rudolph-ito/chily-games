...
describe "Game#valid_plies - range - terrain - block" do
  ...
  let!(:terrain_rule) { create :terrain_rule, variant: variant, terrain_type: terrain_type, block_range_type: 'none' }
  ...
  specify do
    expect( game.valid_plies(piece, piece.coordinate, 'range') ).to match_array [{"x"=>4, "y"=>2}, {"x"=>0, "y"=>2}, {"x"=>2, "y"=>4}, {"x"=>2, "y"=>0}]
  end
end
