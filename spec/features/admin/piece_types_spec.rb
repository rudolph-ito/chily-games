require 'spec_helper'

describe 'admin piece types' do
  before do
    sign_in admin: true
  end

  describe 'index' do
    before { create_list :piece_type, 3 }

    it 'succeeds' do
      visit admin_piece_types_path
    end
  end

  describe 'new/create' do
    it 'creates and redirects' do
      visit new_admin_piece_type_path

      fill_in 'Name', with: 'test'
      attach_file 'Alabaster image', Rails.root.join('spec/support/fake_image.svg')
      attach_file 'Onyx image', Rails.root.join('spec/support/fake_image.svg')
      click_on 'Create'

      expect(page).to have_content 'Piece type was successfully created.'
    end
  end

  describe 'show/edit/update' do
    let(:piece_type) { create :piece_type, name: 'old' }

    it 'updates and redirects' do
      visit admin_piece_type_path(piece_type)
      click_on 'Edit Piece Type'

      fill_in 'Name', with: 'new'
      click_on 'Update'

      expect(page).to have_content 'new'
      expect(page).to_not have_content 'old'
      expect(page).to have_content 'Piece type was successfully updated.'
    end
  end

  describe 'destroy' do
    let(:piece_type) { create :piece_type }

    it 'succeeds' do
      visit admin_piece_type_path(piece_type)
      click_on 'Delete Piece Type'
      page.driver.browser.switch_to.alert.accept

      expect(page).to have_content 'Piece type was successfully destroyed.'
    end
  end
end
