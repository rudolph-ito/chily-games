require 'spec_helper'

describe TerrainTypesController do
  describe 'index' do
    it 'succeeds' do
      get :index
      response.status.should == 200
    end
  end

  describe 'new' do
    context 'when signed in as admin', :signed_in_as_admin do
      it 'succeeds' do
        get :new
        response.status.should == 200
      end
    end

    context 'when signed in', :signed_in do
      it 'returns unauthorized' do
        get :new
        response.should redirect_to root_path
      end
    end

    context 'when not signed in' do
      it 'redirects to login' do
        get :new
        response.should redirect_to new_user_session_path
      end
    end
  end

  describe 'create' do
    let(:valid_attributes) { {
      name: 'test',
      image: Rack::Test::UploadedFile.new('spec/support/fake_image.svg')
    } }

    context 'when signed in as admin', :signed_in_as_admin do
      context 'with valid attributes' do
        it 'creates and redirects' do
          expect {
            post :create, terrain_type: valid_attributes
            response.should redirect_to TerrainType.last
          }.to change(TerrainType, :count).by(1)
        end
      end

      context 'with invalid attributes' do
        it 'does not create and renders new' do
          expect {
            post :create, terrain_type: valid_attributes.merge(name: '')
            response.should render_template 'new'
          }.to change(TerrainType, :count).by(0)
        end
      end
    end

    context 'when signed in', :signed_in do
      it 'redirects to login' do
        expect {
          post :create, terrain_type: valid_attributes
          response.should redirect_to root_path
        }.to change(TerrainType, :count).by(0)
      end
    end

    context 'when not signed in' do
      it 'redirects to login' do
        expect {
          post :create, terrain_type: valid_attributes
          response.should redirect_to new_user_session_path
        }.to change(TerrainType, :count).by(0)
      end
    end
  end

  describe 'show' do
    let(:terrain_type) { create :terrain_type }

    it 'succeeds' do
      get :show, id: terrain_type.id
      response.status.should == 200
    end
  end

  describe 'edit' do
    let(:terrain_type) { create :terrain_type }

    context 'when signed in as admin', :signed_in_as_admin do
      it 'succeeds' do
        get :edit, id: terrain_type.id
        response.status.should == 200
      end
    end

    context 'when signed in', :signed_in do
      it 'redirects to root' do
        get :edit, id: terrain_type.id
        response.should redirect_to root_path
      end
    end

    context 'when not signed in' do
      it 'redirects to login' do
        get :edit, id: terrain_type.id
        response.should redirect_to new_user_session_path
      end
    end
  end

  describe 'update' do
    let(:terrain_type) { create :terrain_type, name: 'old' }

    context 'when signed in as admin', :signed_in_as_admin do
      context 'with valid attributes' do
        it 'updates and redirects to terrain_type' do
          put :update, id: terrain_type.id, terrain_type: { name: 'new' }
          terrain_type.reload.name.should == 'new'
          response.should redirect_to terrain_type
        end
      end

      context 'with invalid attributes' do
        it 'renders edit' do
          put :update, id: terrain_type.id, terrain_type: { name: '' }
          terrain_type.reload.name.should == 'old'
          response.should render_template 'edit'
        end
      end
    end

    context 'when signed in', :signed_in do
      it 'redirects to root' do
        put :update, id: terrain_type.id, terrain_type: { name: 'new' }
        terrain_type.reload.name.should == 'old'
        response.should redirect_to root_path
      end
    end

    context 'when not signed in' do
      it 'redirects to login' do
        put :update, id: terrain_type.id, terrain_type: { name: 'new' }
        terrain_type.reload.name.should == 'old'
        response.should redirect_to new_user_session_path
      end
    end
  end

  describe 'destroy' do
    let!(:terrain_type) { create :terrain_type }

    context 'when signed in as admin', :signed_in_as_admin do
      it 'destroys and redirects to terrain_types' do
        expect{
          delete :destroy, id: terrain_type.id
          response.should redirect_to terrain_types_path
        }.to change(TerrainType, :count).by(-1)
      end
    end

     context 'when signed in', :signed_in do
      it 'redirects to root' do
        expect{
          delete :destroy, id: terrain_type.id
          response.should redirect_to root_path
        }.to change(TerrainType, :count).by(0)
      end
    end

    context 'when not signed in' do
      it 'redirects to login' do
        expect{
          delete :destroy, id: terrain_type.id
          response.should redirect_to new_user_session_path
        }.to change(TerrainType, :count).by(0)
      end
    end
  end
end
