...
function getMockPlayers(numberOfCards: number, bets: (null | number)[]): IOhHeckPlayer[] {
  return bets.map(bet => {
    return {
      userId: 1,
...
describe("BetValidator", () => {
  describe("validateBet", () => {
    const examples: IBetExample[] = [
      ...
      {
        description: 'returns error if last player is betting and total bets equals number of cards',
        players: getMockPlayers(7, [1, 2, null]),
        bet: 4,
        expectedResult: 'Bet cannot be 4 as sum of bets cannot be 7.'
      },
      {
        description: 'returns no error if last player is betting and total bets does not equal number of cards',
        players: getMockPlayers(7, [1, 2, null]),
        bet: 3,
        expectedResult: null
      }
