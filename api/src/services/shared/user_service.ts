...
} from "../oh_heck/data/oh_heck_game_data_service";
import {
  IRummikubGameDataService,
  RummikubGameDataService,
} from "../rummikub/data/rummikub_game_data_service";
import {
  IYanivGameDataService,
  YanivGameDataService,
} from "../yaniv/data/yaniv_game_data_service";
...
export class UserService implements IUserService {
  ...
  async deleteIfNotInGame(): Promise<DeleteIfNotInGameResult> {
    ...
    const deletedUserCount = await this.userDataService.deleteAllExcept(
      Array.from(usersInGames)
    );
  ...
  private async getUsersInOhHeckGames(): Promise<Set<number>> {
    return this.getUsersInGames(async (pageIndex, pageSize) => {
      ...
      for (const game of searchResult.data) {
      ...
      return {
        userIds,
        itemsInPage: searchResult.data.length,
      };
    });
  }
  ...
  private async getUsersInGames(
    getUsersForPageOfGames: (
      pageIndex: number,
      pageSize: number
    ) => Promise<GetUsersForPageOfGamesResult>
  ): Promise<Set<number>> {
    const userIds: Set<number> = new Set();
    const pageSize = 100;
    let pageIndex = 0;
    while (true) {
      ...
      if (pageResult.itemsInPage < pageSize) {
        break;
      }
      pageIndex += 1;
    }
