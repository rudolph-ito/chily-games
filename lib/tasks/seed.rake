namespace :seed do
  ...
  task :variants => :environment do
    ...
    unless Variant.find_by(user_id: admin.id)
      variant = Variant.create!(user_id: admin.id, board_type: 'hexagonal', board_size: 2, piece_ranks: false)
    end

    unless Variant.find_by(user_id: user1.id)
      variant = Variant.create!(user_id: user1.id, board_type: 'square', board_rows: 7, board_columns: 7, piece_ranks: false)
    ...
    unless Variant.find_by(user_id: user2.id)
      variant = Variant.create!(user_id: user2.id, board_type: 'hexagonal', board_size: 4, piece_ranks: true, support_type: 'sum')

      variant.piece_rules.first.update_attributes!(attack_rank: 3)

      variant.piece_rules.create!(
        piece_type: PieceType.find_by(name: 'Dragon'), count: 1, attack_rank: 3, defense_rank: 3,
        movement_minimum: 1, movement_maximum: nil, movement_type: 'orthogonal_line',
        capture_type: 'movement',
      )

      variant.piece_rules.create!(
        piece_type: PieceType.find_by(name: 'Trebuchet'), count: 1, attack_rank: 2, defense_rank: 1,
        movement_minimum: 1, movement_maximum: 1, movement_type: 'orthogonal_line',
        capture_type: 'range',
        range_minimum: 2, range_maximum: 4, range_type: 'orthogonal_line'
      )
      ...
      variant.piece_rules.create!(
        piece_type: PieceType.find_by(name: 'Crossbow'), count: 1, attack_rank: 1, defense_rank: 1,
        movement_minimum: 1, movement_maximum: 1, movement_type: 'orthogonal_line',
        capture_type: 'range',
        range_minimum: 1, range_maximum: 1, range_type: 'diagonal_line'
      )
      ...
      variant.piece_rules.create!(
        piece_type: PieceType.find_by(name: 'Light Horse'), count: 1, attack_rank: 1, defense_rank: 1,
        movement_minimum: 2, movement_maximum: 3, movement_type: 'orthogonal_with_turns',
        capture_type: 'movement'
      )
