...
interface IPlayerSizingData {
  ...
  maxOffset: IPosition;
  currentSize: ISize;
  currentOffset: IPosition;
}

interface IPlayerPositionalData {
  textPosition: IPosition;
  textWidth: number;
  borderPosition: IPosition;
  borderSize: ISize;
  cardPositions: ICardPositionalData[];
}
...
export class YanivTable {
  ...
  async initializeState(game: IGame, currentUserId?: number): Promise<void> {
    ...
    await Promise.all([
      ...
      this.initializeDeck(),
      this.initializeDiscards(game.cardsOnTopOfDiscardPile),
    ]);
  ...
  resize(): void {
    ...
    for (let index = 0; index < this.discardedCards.length; index++) {
      this.updateCardSizeAndPosition(
        ...
        {
          position: this.getDiscardPosition(index, this.discardedCards.length),
          offset: { x: 0, y: 0 },
          rotation: 0,
        },
        false
    ...
    this.users.forEach((userData) => {
      ...
      userData.border.position(positionalData.borderSize);
  ...
  private getDeckPosition(): IPosition {
    return {
  ...
  async initializeDeck(): Promise<void> {
    ...
    cardBack.position(this.getDeckPosition());
  ...
  private getPlayerPositionData(
    ...
    const radians =
      (2 * Math.PI * userData.positionIndex) / numberOfPlayers + Math.PI / 2;
    const position = {
      x: (Math.cos(radians) + 1) * tableXRadius + this.playerOffset,
      y: (Math.sin(radians) + 1) * tableYRadius + this.playerOffset,
    };
    const sizingData = this.getPlayerSizingData(
      userData.userId,
    ...
    return {
      textPosition: {
        x: position.x - sizingData.currentOffset.x,
        y:
          position.y +
          sizingData.padding -
          sizingData.currentOffset.y +
          this.cardHeight * 1.2,
      },
      textWidth: sizingData.currentSize.width,
      borderPosition: {
        x: position.x - sizingData.maxOffset.x,
        y: position.y - sizingData.maxOffset.y,
      },
      borderSize: sizingData.maxSize,
      cardPositions,
    };
  }
  ...
  private getCardPositionalData(
    ...
    const position: IPosition = {
      x:
        playerPosition.x +
        sizingData.padding -
        sizingData.currentOffset.x +
        cardIndex * sizingData.cardSpacer,
      y: playerPosition.y + sizingData.padding - sizingData.currentOffset.y,
    };
  ...
  private updateCardSizeAndPosition(
    ...
    if (animate) {
      ...
      rect.position(positionalData.position);
      rect.offset(positionalData.offset);
      rect.rotation(positionalData.rotation);
    }
  }
}
